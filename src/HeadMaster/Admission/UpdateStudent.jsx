...
export default function UpdateStudent({ setActivePage }) {
    ...
    const captureSnapshot = () => {
        ...
        canvas.toBlob((blob) => {
            ...
            setFormData(p => ({ ...p, [webcamTarget]: file, [`${webcamTarget}Preview`]: preview }));
            stopWebcam();
        }, "image/jpeg", 0.92);
    };

    useEffect(() => { if (webcamOpen) startWebcam(); }, [webcamOpen]);

    const openCamera = (fieldName) => {
        setPhotoPickerTarget(null);
        if (isMobile) { cameraRefs[fieldName].current.click(); }
        else { setWebcamTarget(fieldName); setWebcamOpen(true); }
    };
    ...
    const handlePhoto = (e, fieldName) => {
        ...
        setFormData(p => ({ ...p, [fieldName]: file, [`${fieldName}Preview`]: preview }));
        e.target.value = "";
    };
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const res = await api.get(`/students/by-enrollment/${searchEnrollment.trim()}`);
            const s = res.data;
            ...
            setFormData({
                ...
                studentPhotoPreview: s.studentPhotoUrl || null,
                firstName: s.firstName || "",
                middleName: s.middleName || "",
                lastName: s.lastName || "",
                gender: s.gender !== undefined ? String(s.gender) : "",
                dateOfBirth: toInputDate(s.dateOfBirth),
                placeOfBirth: s.placeOfBirth || "",
                adharNo: s.adharNo || "",
                phoneNo: s.phoneNo || "",
                email: s.email || "",
                studentCast: s.studentCast || "",
                studentCastCertNo: s.studentCastCertNo || "",
                houseNo: s.houseNo || "",
                streetName: s.streetName || "",
                locationVillage: s.locationVillage || "",
                subDistTq: s.subDistTq || "",
                district: s.district || "",
                state: s.state || "",
                pincode: s.pincode || "",
                fatherName: s.fatherName || "",
                fatherAdharNo: s.fatherAdharNo || "",
                fatherPhoneNo: s.fatherPhoneNo || "",
                fatherEmail: s.fatherEmail || "",
                fatherCast: s.fatherCast || "",
                fatherCastCertNo: s.fatherCastCertNo || "",
                fatherPhotoPreview: s.fatherPhotoUrl || null,
                motherName: s.motherName || "",
                motherAdharNo: s.motherAdharNo || "",
                motherPhoneNo: s.motherPhoneNo || "",
                motherEmail: s.motherEmail || "",
                motherCast: s.motherCast || "",
                motherCastCertNo: s.motherCastCertNo || "",
                motherPhotoPreview: s.motherPhotoUrl || null,
                prevSchoolName: s.prevSchoolName || "",
                prevSchoolDiceCode: s.prevSchoolDiceCode || "",
                transferLetterNo: s.transferLetterNo || "",
                transferLetterDate: s.transferLetterDate ? s.transferLetterDate.split("T")[0] : "",
                prevSyllabus: s.prevSyllabus || "",
                prevSchoolCategory: s.prevSchoolCategory || "",
                motherTongue: s.motherTongue || "",
                teachingMedium: s.teachingMedium || "",
                bplCardHolder: s.bplCardHolder || false,
                specialNeed: s.specialNeed || "",
                rationCardNumber: s.rationCardNumber || "",
                exploitGroup: s.exploitGroup || "",
                bhagyaLaxmiBondNo: s.bhagyaLaxmiBondNo || "",
                earlyLateAdmissionReason: s.earlyLateAdmissionReason || "",
            });
            setCurrentStep(1);
            setSaved(false);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setSaved(true);
            showNotif("success", "Student details updated successfully.");
        } catch (err) {
            showNotif("error", err.response?.data?.message || "Failed to update student.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="nsd-container">
            ...
            <div className="nsd-header">
                <div>
                    <h1 className="nsd-title">Update Student Details</h1>
                    <p className="nsd-breadcrumb">Admission / <span>Update Student</span></p>
                </div>
            </div>

            {/* Search bar */}
            <div className="nsd-card" style={{ marginBottom: "1rem" }}>
                ...
                <div className="nsd-card__body">
                    <form style={{ display: "flex", gap: "0.65rem" }} onSubmit={handleSearch}>
                        <input
                            style={{ flex: 1, padding: "0.55rem 0.85rem", border: "1px solid #d1d5db", borderRadius: "7px", fontSize: "0.9rem" }}
                            type="text"
                            placeholder="e.g. SAR00042"
                            value={searchEnrollment}
                            onChange={e => setSearchEnrollment(e.target.value.toUpperCase())}
                            required
                        />
                        <button type="submit" className="nsd-btn nsd-btn--primary" disabled={searchLoading}>
                            {searchLoading ? "Searching…" : "Load Student"}
                        </button>
                        {studentLoaded && (
                            <button type="button" className="nsd-btn nsd-btn--ghost"
                    ...
                    {searchError && (
                        <div style={{ marginTop: "0.5rem", color: "#b91c1c", fontSize: "0.83rem" }}>
                            ❌ {searchError}
                        </div>
                    )}
                    {studentLoaded && (
                        <div style={{ marginTop: "0.5rem", color: "#15803d", fontSize: "0.83rem" }}>
                            ✅ Student loaded — edit details below and click Save.
                        </div>
                    )}
                </div>
            </div>

            {/* Form — only after student loaded */}
            {studentLoaded && (
                <>
                    {/* Stepper */}
                    ...
                    <div className="nsd-form">
                                ...
                                <div className="nsd-card__body">
                                    ...
                                    <div className="nsd-row-1" style={{ marginTop: 14 }}>
                                        <div className="nsd-field">
                                            ...
                                            <PhotoBox label="Change Photo" preview={formData.studentPhotoPreview} fieldName="studentPhoto" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="nsd-actions">
                            <button type="button" className="nsd-btn nsd-btn--ghost"
                                onClick={currentStep === 1 ? () => setStudentLoaded(false) : prevStep}>
                                {currentStep === 1 ? "← Change Student" : "← Back"}
                            </button>
                            <div className="nsd-actions__right">
                                {currentStep < 6 ? (
                                    ...
                                    <button type="button" className="nsd-btn nsd-btn--primary"
                    ...
                    {photoPickerTarget && (
                        <div className="nsd-picker-overlay" onClick={() => setPhotoPickerTarget(null)}>
                            <div className="nsd-picker" onClick={e => e.stopPropagation()}>
                                ...
                                <button type="button" className="nsd-picker__btn"
                                    onClick={() => openCamera(photoPickerTarget)}>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                                        <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" />
                                        <circle cx="12" cy="13" r="4" />
                                    </svg>
                                ...
                                <button type="button" className="nsd-picker__btn"
                                    onClick={() => { browseRefs[photoPickerTarget].current.click(); setPhotoPickerTarget(null); }}>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                                        <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
                                    </svg>
                                    Browse from Files
                                </button>
                                <button type="button" className="nsd-picker__cancel"
                                    onClick={() => setPhotoPickerTarget(null)}>Cancel</button>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="nsd-picker-overlay" onClick={stopWebcam}>
                            <div className="nsd-webcam" onClick={e => e.stopPropagation()}>
                                ...
                                <div className="nsd-webcam__actions">
                                    ...
                                    <button type="button" className="nsd-btn nsd-btn--ghost" onClick={stopWebcam}>Cancel</button>
                                </div>
                            </div>
                        </div>
                    )}
